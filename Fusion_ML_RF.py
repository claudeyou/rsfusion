...
df_list = []
for i, fu_img in enumerate([fusion_list[0]]):
    temp_df = pd.DataFrame()
    for band in fu_img['band_data'].keys():
        temp_df[f'{band}_{i}'] = fu_img['band_data'][band].flatten()
    df_list.append(temp_df)
...
clf = RandomForestClassifier(n_estimators=300, min_samples_split=20, 
...
rf_result = pred.reshape(fusion_list[0]['band_data']['B1'].shape)
...
plt.title('Classified Fusion Image - Single')
plt.tight_layout()
plt.axis('off')
output_path = r'/Users/hoyeong/Desktop/PBML_Project/Image/Fusion_single_def1.jpeg'
...
plt.tight_layout()
plt.axis('off')
output_path = r'/Users/hoyeong/Desktop/PBML_Project/Image/Fusion_single_cmap1.jpeg'
plt.savefig(output_path, dpi=500)
plt.ion()
plt.show()
...
plt.tight_layout()
out_path = r'/Users/hoyeong/Desktop/PBML_Project/Image/Fusion_timeseries_confmat.jpeg'
