...
lgb_clf = lgb.LGBMClassifier(random_state = 100, n_jobs = -1)
lgb_cv = RandomizedSearchCV(lgb_clf, param_distributions = params, cv = 5, 
                            scoring='accuracy', verbose=3, n_jobs = -1)
lgb_cv.fit(X_train, y_train)

print('Best Hyper-Parameters: ', lgb_cv.best_params_)
print('Best Accuracy Score: {:.4f}'.format(lgb_cv.best_score_))

clf = lgb_cv.best_estimator_
...
lgb_result = pred.reshape(fusion_list[0]['band_data']['vv_band'].shape)
...
plt.imshow(lgb_result)
plt.title('Classified Fusion Image - LightGBM')
plt.tight_layout()
plt.axis('on')
output_path = r'/Users/hoyeong/Desktop/PBML_Project/Image/Fusion_LGBM_def2.jpeg'
...
plt.imshow(lgb_result, cmap=cmap)
plt.title('Classified Fusion Image - LightGBM')
...
plt.tight_layout()
plt.axis('on')
output_path = r'/Users/hoyeong/Desktop/PBML_Project/Image/LGBM_cmap2.jpeg'
plt.savefig(output_path, dpi=500)
plt.ion()
plt.show()
...
plt.tight_layout()
out_path = r'/Users/hoyeong/Desktop/PBML_Project/Image/LGBM_confmat.jpeg'
...
print('y_pred2 Test Accuracy: ', accuracy_score(y,y_pred2))

lgb_result2 = y_pred2.reshape(fusion_list[0]['band_data']['vv_band'].shape)

cmap = ListedColormap([cmap_dict[x] for x in sorted(cmap_dict.keys())])
plt.imshow(lgb_result2, cmap=cmap)
plt.title('Classified Fusion Image - LightGBM')
legend_labels = [Patch(facecolor=cmap_dict[key], edgecolor=cmap_dict[key], label=class_dict[key]) for key in cmap_dict.keys()]
plt.legend(handles=legend_labels, bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.)  
plt.tight_layout()
plt.axis('off')
output_path = r'/Users/hoyeong/Desktop/PBML_Project/Image/LGBM_cmap2_Region2.jpeg'
plt.savefig(output_path, dpi=500)
plt.ion()
plt.show()
...
feature_imp = pd.DataFrame({'features': X_train.columns, 'values': clf.feature_importances_})
...
output_path = r'/Users/hoyeong/Desktop/PBML_Project/Image/Feature Importance.jpeg'
plt.savefig(output_path, dpi=500)
plt.ion()
plt.show()